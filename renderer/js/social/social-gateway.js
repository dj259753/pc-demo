...
const SocialGateway = (() => {
  ...
  const localClient = {
    ...
    getActiveVisits: () => typeof SocialBridge.getActiveVisits === 'function' ? SocialBridge.getActiveVisits() : Promise.resolve({ success: true, data: { myself: null, friendsInVisit: [] } }),
  ...
  const remoteFallbackClient = {
    ...
    getActiveVisits: async () => makeMissingRemoteResult('getActiveVisits'),
  ...
  return {
    ...
    getActiveVisits: (payload) => getClient().getActiveVisits(payload),
